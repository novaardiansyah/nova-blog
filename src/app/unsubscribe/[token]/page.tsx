...
import { Trash2, XCircle, Home, Mail, Ghost } from "lucide-react"
    ...
    const unsubscribe = async () => {
      try {
        const [response] = await Promise.all([
          fetch(`/api/blog-subscribers/unsubscribe`, {
            method: "POST",
            headers: {
          ...
          {status === "loading" && (
            <>
              ...
              <p className="text-base md:text-lg text-muted-foreground">
                Mohon tunggu sebentar, kami sedang memproses permintaan berhenti berlangganan Anda.
              </p>
            </>
          )}
            ...
            <>
              <div className="mb-6">
                <div className="w-20 h-20 rounded-full bg-orange-100 dark:bg-orange-900/30 flex items-center justify-center">
                  <Ghost className="w-10 h-10 text-orange-600 dark:text-orange-400" />
                </div>
              </div>
              ...
              <p className="text-base md:text-lg text-muted-foreground max-w-lg leading-relaxed mb-6">
                Kami sedih melihat Anda pergi. Email Anda telah berhasil dihapus dari daftar buletin kami.
                Anda tidak akan lagi menerima update artikel terbaru dari kami.
              </p>
              ...
              <div className="w-full max-w-md bg-card border border-border rounded-xl p-6 mb-8">
                ...
                <div className="pt-4 border-t border-border">
                  <p className="text-sm text-muted-foreground">
                    Berhenti berlangganan pada {formatDate(data.unsubscribed_at)}
                  </p>
                </div>
              </div>
